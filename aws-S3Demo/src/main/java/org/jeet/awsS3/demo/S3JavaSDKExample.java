...
public class S3JavaSDKExample {
	...
	public static void createAndPopulateSimpleBucket() throws Exception {
		...
		String newBucketName = "jitendra" + System.currentTimeMillis();
		s3Client.createBucket(newBucketName);

		final String fileName = "sometext.txt";

		System.out.println(S3JavaSDKExample.class.getResource(fileName).toURI());
		
		
		File file = new File(S3JavaSDKExample.class.getResource(fileName).toURI());

		/*
		 * remember this is a new bucket and "folders" don't exist in S3, they
		 * are logical entities derived from the path specified in the key. S3
		 * is just a key value store.
		 * 
		 * they are created on the fly when we upload an object with a specific
		 * key path
		 * 
		 * Also, the folder setting in the console on the S3 folder for server
		 * side encryption is a slightly misleading instruction to encrypt the
		 * selected resources - it does NOT set a persistent setting on all
